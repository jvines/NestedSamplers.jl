...
function fit(E::Type{<:Ellipsoid{R}}, x::AbstractMatrix{S}; pointvol = 0) where {R,S}
    ...
    # Ensure cov is well-conditioned and get transformation matrix
...
"""
    make_eigvals_positive(cov::Symmetric) -> (cov, A)

Ensure covariance matrix is well-conditioned by capping its condition
number at 1e10. Returns the conditioned covariance and its inverse
(the precision matrix), both as Symmetric.

Based on PR #81 (aplavin) and dynesty's _make_eigvals_positive.
"""
